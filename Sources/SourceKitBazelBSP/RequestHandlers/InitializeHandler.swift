...
final class InitializeHandler {
    ...
    func makeInitializedConfig(
        ...
        let devDir = try commandRunner.run("xcode-select --print-path").toStr()
        let sdkRoot = try commandRunner.run("xcrun --sdk iphonesimulator --show-sdk-path").toStr()
    ...
    func getToolchainPath(with commandRunner: CommandRunner) throws -> String {
        ...
        let swiftPath = try commandRunner.run("xcrun --find swift").toStr()

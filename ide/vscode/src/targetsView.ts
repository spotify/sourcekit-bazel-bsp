    ...
    resolveWebviewView(
        ...
        webviewView.webview.onDidReceiveMessage((message) => {
            if (message.type === "ready") {
                ...
                const target = this.targets?.find(t => t.label === message.label);
                if (target && this._onLaunchTarget) {
                    this._onLaunchTarget(target);
                }
            } else if (message.type === "launchTargetWithoutDebugging") {
                const target = this.targets?.find(t => t.label === message.label);
                if (target && this._onLaunchTargetWithoutDebugging) {
    ...
    <script>
        ...
        function renderTargets() {
            ...
            targetList.innerHTML = sortedTargets.map(target => {
                ...
                const launchButton = target.type === 'app' && target.canRun
                    ...
                    : '';
                const launchWithoutDebuggingButton = target.type === 'app' && target.canRun
                    ...
                    : '';
                const testWithoutDebuggingButton = target.type === 'test' && target.canRun
                    ? '<button class="action-button" data-action="testWithoutDebugging" data-label="' + escapeHtml(target.label) + '" title="Run tests without debugging"><span class="codicon codicon-play"></span></button>'
                    : '';

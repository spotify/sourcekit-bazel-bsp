...
function getLaunchArgs(config: Configuration, target: ProcessedTarget): string {
    let launchArgs = config.launchArgs;
...
function createBuildTask(extensionPath: string, config: Configuration, target: ProcessedTarget): vscode.Task {
    ...
    const task = new vscode.Task(
        ...
        `Build ${target.label}`,
        "sourcekit-bazel-bsp",
        new vscode.ShellExecution(getScriptPath(extensionPath, "lldb_build.sh"), {
            env: {
...
function createLaunchTask(extensionPath: string, config: Configuration, target: ProcessedTarget, withoutDebugging: boolean = false): vscode.Task {
...
function createTestTask(extensionPath: string, config: Configuration, target: ProcessedTarget, withoutDebugging: boolean = false): vscode.Task {
...
export class BuildTaskProvider implements vscode.TaskProvider {
    ...
    private rebuildTasks(): void {
        ...
        for (const target of this.targets) {
            this.tasksByLabel.set(target.label, createBuildTask(this.extensionPath, this.config, target));
            if (target.type === "app" && target.canRun) {
                this.launchTasksByLabel.set(getLaunchTaskLabel(target.label), createLaunchTask(this.extensionPath, this.config, target));
                this.launchWithoutDebuggingTasksByLabel.set(getLaunchTaskLabel(target.label, true), createLaunchTask(this.extensionPath, this.config, target, true));
            }
            if (target.type === "test" && target.canRun) {
                this.testTasksByLabel.set(getTestLaunchTaskLabel(target.label), createTestTask(this.extensionPath, this.config, target));
                this.testWithoutDebuggingTasksByLabel.set(getTestLaunchTaskLabel(target.label, true), createTestTask(this.extensionPath, this.config, target, true));
            }
        }
